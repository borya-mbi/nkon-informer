...
4. **Deep Verification** (якщо увімкнено):
   - Перевіряє реальну кількість, додаючи 30000 шт. в кошик і парсячи помилку.
   - Виявляє "фантомні" товари, коли сайт показує наявність, але фактично залишок = 0.
   - Якщо поле кількості скидається без повідомлення про помилку - це розцінюється як 0 шт.
