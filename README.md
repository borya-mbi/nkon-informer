...
- ✅ **Повний звіт** кожного запуску - всі доступні товари
- ✅ Відстеження змін:
  - 🆕 Нові товари
  - ❌ Видалені товари
  - 💰 Зміни цін
  - 🔄 Зміна статусу (Pre-order ↔ In Stock)
...
Скрипт інтелектуально визначає тип ціни:
1. **Пріоритет:** Скрипт шукає ціну **без ПДВ** (Excl. Tax). Якщо вона знайдена, відображається "чиста" ціна (наприклад `€52.95`).
2. **Fallback:** Якщо ціну без ПДВ не знайдено, береться основна ціна (зазвичай з ПДВ) і до неї додається помітка `(VAT)` (наприклад `€79.95 (VAT)`).
...
# Відредагуйте config.json в VS Code або Notepad++
...
> [!TIP]
> **Клікові посилання:** Натисніть на ємність товару (наприклад, `[280Ah]`) щоб одразу перейти на сторінку цього товару.
...
```cron
0 9 * * * cd /шлях/до/nkon-informer && /usr/bin/python3 nkon_monitor.py >> nkon_cron.log 2>&1
```
...
├── config.json               # Конфігурація (НЕ комітити!)
...
- **НЕ комітьте** `config.json` з токенами в Git
- Використовуйте `.gitignore` (вже включено)
- Обмежте права доступу: `chmod 600 config.json`
