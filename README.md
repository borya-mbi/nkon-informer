...
Коли змін не виявлено, надсилається повний перелік товарів. Кожен рядок містить: ємність (клікове посилання), Grade, назву, ціну та статус. Для pre-order додається дата доставки. Внизу  - час оновлення (🔄 = редагування існуючого повідомлення).
...
Коли виявлено відмінності від попереднього запуску, надсилається **нове** повідомлення з секціями: ✨ нові товари, 💰 зміни цін (з відсотком: `🔴+31.4%` або `🟢-10.1%`), 📦 зміни статусу/дати доставки, ❌ видалені. Внизу - час (🆕 = нове повідомлення).
...
💰 Зміни цін (1):
• 324Ah 🅰️ Grade A | REPT Battero 72174L4 €40.00 → €52.55 (🔴+31.4%)

📦 Зміни статусу (1):
• 📦 334Ah 🅰️➖ Grade A- | Eve LF334 Single stud
  └──▷ 01-01-2099 → 12-03-2026 | €59.20
...
4. Regex `(\d+)Ah` витягує ємність
5. Фільтрує товари ≥200Ah
6. Порівнює з `state.json` (попередній запуск)
7. Виявляє зміни (нові, видалені, ціна, статус)
8. Відправляє звіт в Telegram
9. Зберігає новий state
