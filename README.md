...
- 📊 **Розумні сповіщення:** 
  - **Granular Recipients:** Тепер можна налаштувати окремі параметри (min Ah, тип звіту) для кожного отримувача окремо через JSON-конфігурацію.
  - **Quiet Mode:** Автоматичне вимкнення звуку для сповіщень у нічний час (налаштовується через `QUIET_HOURS_START/END`).
  - **Dual Message Flow:** При змінах - два повідомлення: зміни (зі звуком 🔔) + "Новий стан" (беззвучно 🔕)
  - **Smart Heartbeat:** Сповіщення зі звуком тільки у задані години (наприклад, `8:00, 12:00, 16:00`).
  - **Automatic Cooldown:** Система автоматично вираховує час тиші на основі вашого розкладу. Це запобігає дублюванню сповіщень (наприклад, якщо за 10 хв до планового heartbeat прийшло сповіщення про зміну ціни, heartbeat буде пропущено).
  - **Smart Message Editing:** Повідомлення "Новий стан" автоматично редагується на "Без змін" (оновлюється час), не спамить канал. Коли з'являються реальні зміни, старе повідомлення оновлюється фінальними дельтами перед надсиланням нового звіту.
  - **Improved Robustness:** Підтримка різних роздільників (`,` або `;`) у конфігурації `HEARTBEAT_TIMES`.
...
- 🔄 **Persistent Quiet Mode:** Система запам'ятовує товари, про які вже було надіслано сповіщення з малою кількістю. Якщо такий товар зникає і знову з'являється з малим залишком (менше `RESTOCK_THRESHOLD`), повторне сповіщення "Новий товар" не надсилається. Це запобігає спаму при коливаннях залишків 1-2 шт. Повноцінне сповіщення прийде тільки при суттєвому поповненні складу.
...
- ✅ **Повне покриття каталогу:** Розумна пагінація автоматично завантажує всі сторінки категорії.
- ✅ Автоматичний парсинг сайту з підтримкою JavaScript (Selenium)
- ✅ **Гнучкий парсинг ємності:** Підтримка форматів `280Ah`, `314Ah`, а також кириличного `230Аг`.
- ✅ **Кириличні грейди:** Розпізнавання `Група А`, `Клас Б` нарівні з `Grade A/B`. Автоматична нормалізація кирилиці у латиницю для коректного відображення емодзі (🅰️/🅱️).
- ✅ Telegram сповіщення з детальною статистикою
- ✅ **Захист від збоїв (Fallback):** Автоматичне використання кешованих даних при тимчасових помилках парсингу (дати/залишки)
- ✅ **Deep Verification:** Розумне виявлення реальних залишків. Алгоритм адаптивного пошуку (бінарний пошук) з автоматичним очищенням кошика після успішних спроб та захистом від застарілих повідомлень інтерфейсу. Це дозволяє точно визначати складські ліміти навіть для великих партій товарів.
...
| **`.env` файл** | `.env` у папці | `.env` у папці | Локально, рекомендується для Docker/VPS |
| **Recipients Wizard**| `setup_env.ps1` | `setup_env.sh` | Інтерактивний майстер налаштування отримувачів |
...
Утиліта `verify_env.py` перевіряє `.env` на помилки та допомагає їх виправити:

```bash
# Перевірити конфігурацію
python verify_env.py

# Автовиправлення помилок (наприклад, пропущені лапки у JSON)
python verify_env.py --fix

# Переформатувати JSON у зручний багаторядковий формат
python verify_env.py --beautify
```

Скрипт перевіряє:
- ✅ Наявність обов'язкових змінних (`TELEGRAM_BOT_TOKEN`)
- ✅ Валідність JSON у `TELEGRAM_CONFIG_JSON`
- ✅ Наявність полів `chat_id`, `type`, `name` у кожного отримувача
...
- `MIN_CAPACITY_AH`: Глобальна мін. ємність (використовується як default).
- `PRICE_ALERT_THRESHOLD`: Поріг зміни ціни % (default: 5).
- `FETCH_DELIVERY_DATES`: Чи збирати дати доставки (default: `true`).
...
Коли змін не виявлено, попереднє повідомлення "Новий стан" (або "Без змін") **редагується** на місці. Кожен рядок містить: ємність (клікове посилання), Grade, назву, ціну, статус та кількість на складі `[N шт]` або статус `[В наявності]` (якщо точна кількість не збирається). Якщо залишки змінились з моменту останнього нового повідомлення, показується кумулятивний diff (наприклад, `(-13+292)`). Внизу - час оновлення (🔄 = редагування існуючого повідомлення).
...
### 🧹 Утиліта для очищення чату

При тестуванні або зміні конфігурації в чаті можуть накопичуватися непотрібні повідомлення. Для цього створена утиліта:
- **`cleanup_telegram.py`**: Дозволяє видаляти повідомлення бота за списком ID або діапазоном.
- **Support**: Працює з усіма налаштованими отримувачами (групи, канали). Якщо ID не знайдено в `state.json`, спробує видалити його у всіх чатах.

  ```bash
  python cleanup_telegram.py
  ```
  Скрипт автоматично запропонує ID останніх базових повідомлень зі `state.json`.
...
Додайте рядок (щогодини - рекомендовано для Smart Heartbeat):
```cron
0 7-21 * * * cd /шлях/до/nkon-informer && ./venv/bin/python nkon_monitor.py >> nkon_cron.log 2>&1
```

Приклади розкладу:
- `0 7-21 * * *` - щогодини з 7 ранку до 9 вечора (РЕКОМЕНДОВАНО)
- `0 9,15,21 * * *` - тричі на день (9:00, 15:00, 21:00)
- `0 9 * * *` - щодня о 9:00

### 7. Керування логами (Logrotate)

Проект генерує два типи логів:
- `nkon_monitor.log`: Автоматично ротується Python-скриптом (10MB x 5 файлів).
- `nkon_cron.log`: Лог виконання через cron, який потребує системної ротації.

Щоб налаштувати автоматичну очистку `nkon_cron.log` (зберігання за 7 днів):
```bash
...
├── cleanup_telegram.py       # Утиліта для очищення повідомлень
├── verify_env.py             # Перевірка та виправлення .env конфігурації
...
### 📝 Логи адаптивного пошуку залишків

Логи скрипта тепер детально відображають процес пошуку залишків:
- `[iter 1/12]` — номер ітерації адаптивного пошуку (макс. 12).
- `👍 1317 доступно. Шукаємо БІЛЬШЕ...` — товар успішно додався, бот пробує більшу кількість.
- `👎 1347 забагато. Шукаємо МЕНШЕ...` — помилка "requested qty", бот пробує меншу кількість.
- `📌 ERROR '1332' точніше за SUCCESS` — знайдено точний ліміт у повідомленні сайту, який пріоритетніший за попередні успішні спроби.
- `🛒 Очищення кошика...` — бот видаляє товари після кожного успіху для точності наступної спроби.
...
3. **BeautifulSoup** парсить HTML за CSS селекторами (якщо знайдено кнопку "Next", скрипт автоматично завантажує наступні сторінки):
   - `li.product-item` - контейнери товарів
   - `a.product-item-link` - назви
   - `.btn--cart` - статус (зелена=In Stock, синя=Pre-order)
    - Автоматично коригує статус: `in_stock` з датою доставки -> `preorder`; 0 шт -> `out_of_stock`.
5. Товари з нульовим залишком автоматично видаляються зі звіту та потрапляють у блок "❌ Видалені".
6. **Regex** `(\d+)Ah` та `(\d+)Аг` витягує ємність (підтримує англійську та українську версії).
6a. **Grade Detection:** Regex `[A-BА-Б]` з ключовими словами `Grade`/`Клас`/`Група` витягує грейд. Кирилиця нормалізується: А->A, Б->B.
7. Фільтрує товари ≥200Ah.
...
Дозволено вільне використання та модифікація коду для **особистих потреб** (наприклад, для власного моніторингу).

⛔ **Заборонено:**
- Використання цього коду для створення публічних дзеркал, каналів або ботів, що дублюють функціонал основного каналу.
- Комерційне використання та перепродаж даних без згоди автора.

Всі права на ідею та реалізацію належать автору.

---

## 🤝 Розвиток та співпраця

Хочете покращити бота або додати моніторинг **інших магазинів**?
Ми відкриті до Pull Requests! Якщо ви напишете парсер для нового сайту, ми готові запустити для нього окремий канал і розвивати його разом.
